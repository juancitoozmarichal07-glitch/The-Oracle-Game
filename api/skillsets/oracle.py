...
class Oracle:
            ...
            for i in range(len(RETRY_DELAYS) + 1):
                try:
                    ...
                    if i < len(RETRY_DELAYS):
                        ...
                        print(f"       No más reintentos para '{model}'.")
    ...
    def _verificar_adivinanza(self, datos_peticion):
        ...
        if normalizar(adivinanza_jugador) == normalizar(nombre_secreto):
            ...
            self.estado_animo -= 1 # Pequeña penalización por fallo
            return {"resultado": "derrota"}

    async def _procesar_pregunta(self, datos_peticion):
        # ... (El resto de la lógica de esta función es la que te pasé antes,
        # usando el prompt v23 y la memoria a largo plazo)
        if not self.personaje_actual_dossier: return {"error": "El juego no se ha iniciado."}
    ...
    async def _crear_y_guardar_nuevo_personaje(self):
        # ... (código idéntico a la versión anterior)
        # NOTA: Esta parte podría mejorarse para usar un prompt de creación dedicado
        return None # Simplificado por ahora
